...
describe('UniApp client transport', () => {
  ...
  it('rejects the original callback failure and validates the path first', async () => {
    ...
    await expect(
      transport({ path: '/items', method: 'GET', headers: headers() })
    ).rejects.toBe(failure);
    expect(request).toHaveBeenCalledOnce();

    const invalidRequest = vi.fn((options: UniAppClientRequestOptions) =>
      options.success?.({ data: true })
    );
    const invalidTransport = createUniAppClientTransport({
      baseUrl: 'https://admin.example/',
