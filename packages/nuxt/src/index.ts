...
import type { ClientHeaders, ClientTransport, ClientTransportRequest } from '@peanut-admin/client'
...
const invalidForwardContext = (): Error => new Error('NUXT_SSR_FORWARD_CONTEXT_INVALID')
...
): boolean => patterns.length === 0 || patterns.some((rawPattern) => {
...
export const createNuxtSsrForwardHeaders = (
  ...
  if (!trustedHost(request, options.trustedHosts ?? [])) throw invalidForwardContext()
