...
import type { TargetCandidate, TypedTarget } from '@peanut-admin/vue';
...
const targetKey = (target: TypedTarget): string =>
  JSON.stringify([
...
export const TargetSelector = defineComponent({
  ...
  emits: {
    'update:modelValue': (targets: readonly TypedTarget[]) =>
  ...
  setup(props, { emit }) {
    ...
    return () =>
      h('div', { class: 'pa-target-selector' }, [
        h(
          SelectComponent,
          {
