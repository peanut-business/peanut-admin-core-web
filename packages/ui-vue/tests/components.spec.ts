...
describe('admin shell components', () => {
  ...
  it('emits only host commands and trusted local navigation paths', async () => {
    ...
    await wrapper
      .get('.pa-shell-sidebar a[href="/app/members"]')
      .trigger('click');
    await wrapper.get('.pa-shell__collapse').trigger('click');
    await wrapper
      .findAll('button')
      .find((button) => button.text() === 'Switch tenant')
      ?.trigger('click');
    await wrapper
      .findAll('button')
    ...
    expect(
      wrapper
        .findAll('a')
        .some((link) => link.attributes('href') === '/\\evil.test')
    ).toBe(false);
    expect(
      wrapper
        .get('.pa-shell-navigation__link[href="/app/members"]')
  ...
  it('exposes accessible collapse, Drawer, identity, breadcrumb, and command controls', async () => {
    ...
    expect(wrapper.get('.pa-shell__collapse').attributes('aria-label')).toBe(
      'Collapse navigation'
    );
    expect(wrapper.get('.pa-shell-breadcrumb').attributes('aria-label')).toBe(
      'Breadcrumb'
    );
    expect(wrapper.get('.pa-shell-identity').attributes('aria-label')).toBe(
      'Current identity'
    );
    ...
    expect(
      document.querySelector('.mobile-navigation-drawer .el-drawer__close-btn')
    ).not.toBeNull();
  ...
  it('keeps state and content components accessible', () => {
    const forbidden = mount(ForbiddenState, {
      props: { requestId: 'req_test_3' },
    });
    const content = mount(PageContent, {
      slots: { default: '<button>Command</button>' },
    });
