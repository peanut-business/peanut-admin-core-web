...
describe('Admin Web override registry', () => {
  it('resolves package defaults and application replacements with immutable source metadata', () => {
    ...
    expect(defaults.get('peanut.shell.component.header')).toBe(
      'package-header'
    );
    ...
    expect(application.resolve('peanut.shell.component.header').source).toBe(
      'application'
    );
  });
  ...
  it('fails closed for an undeclared resolution', () => {
    ...
    expect(() => registry.resolve('peanut.shell.component.missing')).toThrow(
      'ADMIN_OVERRIDE_SLOT_UNKNOWN'
    );
  });
});
