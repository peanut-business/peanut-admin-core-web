...
describe('module and operation target state', () => {
  ...
  it('resolves only build-time registered menu route names', () => {
    ...
    expect(registry.resolve('example.work-item.list')?.path).toBe(
      '/app/work-items'
    );
  ...
  it('uses the same module key grammar as the backend runtime', () => {
    ...
    ).toThrow('ADMIN_MODULE_KEY_INVALID');
  });
  ...
  it('keeps same-looking IDs separate by operation and target type', () => {
    ...
    targets.replace(queueScope, [
      {
        target_resource_key: 'example.queue',
        target_role: 'primary',
        target_id: '9001',
        label: 'Queue',
      },
    ...
    expect(targets.selected(projectScope)).toEqual([
      {
        target_resource_key: 'example.project',
        target_role: 'primary',
        target_id: '9001',
    ...
    expect(targets.selected(queueScope)).toEqual([
      {
        target_resource_key: 'example.queue',
        target_role: 'primary',
        target_id: '9001',
  ...
  it('rejects multiple primary targets for a one-required operation', () => {
    ...
    expect(() => targets.selectionForRequest(scope)).toThrow(
      'TARGET_SELECTION_CARDINALITY_INVALID'
    );
  });
  ...
  it('disposes every registered store even when one disposer fails', async () => {
    const first = vi.fn(async () => {
      throw new Error('STORE_DISPOSAL_FAILED');
    });
    const second = vi.fn();
    const unregisterFirst = registerTenantDisposer(
      'example.work-item.first',
      first
    );
    const unregisterSecond = registerTenantDisposer(
      'example.work-item.second',
      second
    );
