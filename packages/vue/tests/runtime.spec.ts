...
describe('runtime error mapping', () => {
  it.each([
    ...
    ).toMatchObject({ kind: expected, code, requestId: 'req_test' });
  });

  it('maps protected-origin failures without exposing configured authorities', () => {
    expect(
      mapAdminRuntimeError(new Error('API_ORIGIN_MISMATCH'), 'platform')
    ).toEqual({
      kind: 'configuration',
      audience: 'platform',
