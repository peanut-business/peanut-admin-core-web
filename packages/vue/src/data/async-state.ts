...
> {
  ...
  readonly load: (query: AsyncListQuery<TFilters>) => Promise<AsyncListPage<TItem>>
...
export const useAsyncList = <
  TItem,
  TFilters extends Record<string, unknown>,
>(options: AsyncListOptions<TItem, TFilters>): AsyncListState<TItem, TFilters> => {
  ...
  const load = async (page = pagination.page): Promise<boolean> => {
    ...
    try {
      ...
      pagination.pageSize = positiveInteger(result.pageSize, pagination.pageSize)
