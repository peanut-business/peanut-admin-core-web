...
export type AdminModuleShellSlotName =
  | 'header-context'
  | 'header-actions'
  | 'sidebar-footer'
  | 'page-tools';
...
export const defineAdminModule = <T extends AdminModuleContribution>(
  contribution: T
): T => {
  ...
  for (const route of contribution.routes) {
    ...
    if (
      route.access.moduleKey !== contribution.key ||
      route.access.permissionKeys.length === 0
    ) {
  ...
  for (const store of contribution.stores ?? []) {
    if (
      !store.key.startsWith(`${contribution.key}.`) ||
      storeKeys.has(store.key)
    ) {
  ...
  for (const slot of contribution.shellSlots ?? []) {
    if (
      slots.has(slot.name) ||
      (slot.access !== undefined && slot.access.moduleKey !== contribution.key)
    ) {
