...
const scopeKey = (scope: OperationTargetScope): string =>
  JSON.stringify([
    scope.moduleKey,
    scope.resourceKey,
    scope.operation,
...
const normalizeCandidates = (
  ...
  return candidates.map((candidate) => {
    if (
      candidate.target_resource_key !== scope.targetResourceKey ||
...
export const useOperationTargets = defineStore(
  'peanut-admin-operation-targets',
  {
    ...
    actions: {
      replace(
        ...
        const available = new Set(
          normalized.map((candidate) => candidate.target_id)
        );
        ...
        this.entries[key] = {
          ...
          selectedIds:
            normalized.length === 1 ? [normalized[0]!.target_id] : retained,
        };
        this.generation += 1;
      },
      select(
        scope: OperationTargetScope,
        targets: readonly TypedTarget[]
      ): void {
        const key = scopeKey(scope);
        ...
        const available = new Set(
          entry.candidates.map((candidate) => candidate.target_id)
        );
        const selected = new Set<string>();
        for (const target of targets) {
          if (
            target.target_resource_key !== scope.targetResourceKey ||
        ...
        if (
          (cardinality === 'none' && selected.size > 0) ||
      ...
      clearModule(moduleKey: string): void {
        this.entries = Object.fromEntries(
          Object.entries(this.entries).filter(
            ([, entry]) => entry.scope.moduleKey !== moduleKey
          )
        );
        this.generation += 1;
      },
