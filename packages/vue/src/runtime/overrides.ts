...
export type AdminOverrideResolutionMetadata = Omit<
...
> = Extract<Slots[number], { readonly key: Key }> extends infer Slot
  ? Slot extends AdminOverrideSlot<infer Value>
    ? Value
    : unknown
  : unknown;

type SlotKey<Slots extends readonly AdminOverrideSlot[]> =
  Slots[number]['key'] & string;

export interface AdminOverrideRegistry<
  Slots extends readonly AdminOverrideSlot[] = readonly AdminOverrideSlot[]
> {
  resolve<Key extends SlotKey<Slots>>(
    key: Key
  ): AdminOverrideResolution<SlotValue<Slots, Key>>;
  get<Key extends SlotKey<Slots>>(key: Key): SlotValue<Slots, Key>;
  diagnostics(): readonly AdminOverrideResolutionMetadata[];
}

const overrideKinds: readonly AdminOverrideKind[] = [
  'service',
  'component',
  'page',
  'route',
...
const validateKey = (value: unknown, kind: unknown): string => {
  if (typeof value !== 'string' || !overrideKeyPattern.test(value)) {
    return fail(
      'ADMIN_OVERRIDE_SLOT_KEY_INVALID',
      typeof value === 'string' ? value : 'unknown'
    );
  }
  if (
    typeof kind !== 'string' ||
    !overrideKindSet.has(kind as AdminOverrideKind)
  ) {
...
const validateVersion = (value: unknown): string => {
  if (typeof value !== 'string' || !contractVersionPattern.test(value)) {
    return fail(
      'ADMIN_OVERRIDE_CONTRACT_VERSION_INVALID',
      typeof value === 'string' ? value : 'unknown'
    );
  }
  return value;
...
): readonly AdminOverrideResolutionMetadata[] =>
  Object.freeze(metadata.map((entry) => freezeMetadata(entry)));

export const defineAdminOverrideSlot = <
  Value,
  Slot extends AdminOverrideSlot<Value>
>(
  slot: Slot
): Slot => slot;
...
export const createAdminOverrideRegistry = <
  Slots extends readonly AdminOverrideSlot[]
>(
  input: AdminOverrideRegistryInput<Slots>
): AdminOverrideRegistry<Slots> => {
  ...
  for (const rawSlot of input.slots) {
    ...
    if (
      rawSlot.validate !== undefined &&
      typeof rawSlot.validate !== 'function'
    ) {
      fail('ADMIN_OVERRIDE_VALIDATOR_INVALID', key);
    }
    const slot = rawSlot as unknown as AdminOverrideSlot;
    if (
      slot.validate !== undefined &&
      !validatorAccepts(slot.validate, slot.defaultValue)
    ) {
      fail('ADMIN_OVERRIDE_DEFAULT_INVALID', key);
    }
  ...
  const metadata = [...values.values()].map(
    ({ key, kind, contractVersion, source }) =>
