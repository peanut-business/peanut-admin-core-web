...
const normalizedDecodedResult = (
  value: ClientDecodeResult
): ClientDecodeResult => {
  const kind = decodeKind(value);
  if (kind === 'success')
    return { kind: 'success', data: (value as ClientDecodeSuccess).data };
...
class PortableClientHeaders implements ClientHeaders {
  ...
  set(name: string, value: string): void {
    if (
      !validHeaderName.test(name) ||
      typeof value !== 'string' ||
      invalidHeaderValue.test(value)
    ) {
...
const requestHeaders = (
  headers: ClientRequestHeaders | undefined
): ClientHeaders => {
...
const requestError = (
  ...
  new ClientRequestError(
    kind,
    safeErrorCode(code, `CLIENT_${kind.toUpperCase()}_ERROR`),
