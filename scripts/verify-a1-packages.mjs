...
if (version[0] !== 22 || version[1] < 12) {
  throw new Error(
    `Node 22.12 or newer within major 22 is required; found ${process.versions.node}`
  );
}
...
for (const name of packages) {
  ...
  if (
    manifest.name !== `@peanut-admin/${name}` ||
...
run('pnpm', ['run', 'build']);
const testInventory = join(
  tmpdir(),
  `peanut-web-core-tests-${process.pid}.json`
);
...
const tgzs = readdirSync(artifacts)
  .filter((name) => name.endsWith('.tgz'))
  .sort();
if (tgzs.length !== packages.length)
  throw new Error(`Expected six tgz files; found ${tgzs.length}`);

const forbiddenClientRuntime =
  /(?:from\s+['"](?:vue|element-plus)|\b(?:window|document|localStorage|sessionStorage|navigator|BroadcastChannel|HTMLElement)\b)/;
for (const relative of [
  ...
  if (forbiddenClientRuntime.test(contents))
    throw new Error(`Client runtime boundary violation in ${relative}`);
}

for (const tgz of tgzs) {
  const manifest = JSON.parse(
    output('tar', ['-xOf', join(artifacts, tgz), 'package/package.json'])
  );
  ...
  for (const required of [
    ...
    if (!contents.split('\n').includes(required))
      throw new Error(`${tgz} is missing ${required}`);
  }
}
...
verifyConsumer(consumer, () => {
  ...
  run(join(consumer, 'node_modules', '.bin', 'vite'), ['build'], {
    cwd: consumer,
  });
  console.log(
    JSON.stringify({
