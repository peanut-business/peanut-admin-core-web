...
function consume(source) {
  ...
  assert.ok(owned !== '..' && !owned.startsWith('..' + sep), 'Use this checkout-owned TMPDIR');
...
test('native ESM consumer preserves independent refresh scopes', () => {
  ...
  `);
  assert.deepEqual(JSON.parse(output), { calls: 2, values: ['fresh-tenant-a', 'fresh-tenant-a', 'fresh-tenant-b'] });
});
